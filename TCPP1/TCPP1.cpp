...
void grade_input(string names[N], int grades[N], int& mins, int& maxs, size_t& maxw, int& ecnt) {
	int i;
	string nsuf, re;
	for (i = 0; i < N; i++) {
		if (names[i].back() == 's')
	...
	if (ecnt != 0) {
		...
		if (re == "yes") {
			ecnt = 0;
			grade_input(names, grades, mins, maxs, maxw, ecnt);
		}
		else
			if (re == "no")
				return; //placeholder
...
void test_save(string names[N], int grades[N], int& maxw) {
	int i;
	string flnm, file;
	ofstream output;
	cout << endl;
	...
	output.open(file);
	for (i = 0; i < N; i++)
		output << names[i] << ' ' << grades[i] << ' ' << endl;
	output.close();
	cout << "The file was saved as '" << file << "'.";
	cout << endl;
}
void test_open(string names[N], int grades[N], int& maxw) {
	int i;
	string flnm, file, x, word = "";
	ifstream input;
	cout << "Which test would you like to open?" << endl;
	cout << ">";
	cin >> flnm;
	file = flnm + ".txt";
	input.open(file);
...
void mainmenu(string names[N], string mdir, int& rep, int& log, int& start, string cusername, int& ch, int& mins, int& maxs, int& ming) {
	int op;
	cout << "\t\t*************************************************************" << endl;
	cout << "\t\t*                         MAIN MENU                         *" << endl;
	cout << "\t\t*                          1.TESTS                          *" << endl;
	...
	switch (op) {
		...
		case 6:
			...
			break;
		default:
			mainmenu(names, mdir, rep, log, start, cusername, ch, mins, maxs, ming);
			break;
	}
	return;
}
int main() {
	...
	char* buffer = {};
	int grades[N], mins(0), maxs(0), ming(0), ecnt(0), start, rep(1), log(0), att(3), maxlen(8), ch(0);
	size_t maxw(0);
	ifstream input;
	ofstream output;
